...
- **Automatic Database Backups**: Daily pg_dump backups saved to `backups/` folder. Runs on server startup and every 24 hours. Keeps the last 7 backups (older ones auto-deleted). Files can be downloaded from the Files panel and restored with `psql "$DATABASE_URL" < backups/<filename>.sql`.
