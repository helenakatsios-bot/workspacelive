...
export async function importCastlePriceList() {
  const csvPath = path.join(
    process.cwd(),
    "attached_assets/CASTLE_&_THINGS_OFFICAL__1771986640733.csv"
  );

  if (!fs.existsSync(csvPath)) {
    console.log("Castle & Things CSV not found, skipping");
    return;
  }
  ...
  console.log(`Castle & Things CSV: Parsed ${rows.length} rows`);
  ...
  try {
    ...
    const existingPL = await client.query(
      "SELECT id FROM price_lists WHERE name = 'Castle & Things'"
    );
    if (existingPL.rows.length > 0) {
      ...
      if (parseInt(existingPrices.rows[0].cnt) > 0) {
        console.log("Castle & Things prices already imported, skipping");
        await client.query("ROLLBACK");
        return;
      }
    } else {
      const newPL = await client.query(
        `INSERT INTO price_lists (name, description, is_default) VALUES ('Castle & Things', 'Castle & Things pricing', false) RETURNING id`
      );
      priceListId = newPL.rows[0].id;
    }
    console.log(`Castle & Things price list ID: ${priceListId}`);
    ...
    for (const [key, groupRows] of grouped) {
      ...
      if (!product) {
        ...
        const generatedSku = firstRow.sku || `CT-${Date.now()}-${newProductCount}`;
    ...
    console.log(
      `Castle & Things import complete: ${insertedCount} prices inserted, ${skippedCount} skipped, ${newProductCount} new products created`
    );
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("Castle & Things import error:", error);
    throw error;
  } finally {
    client.release();
  }
...
if (process.argv[1]?.includes("import-castle-pricelist")) {
  importCastlePriceList()
    .then(() => process.exit(0))
