...
export async function importBedroomPriceList() {
  const csvPath = path.join(
    process.cwd(),
    "attached_assets/Price_for_replit_the_bedroom_CSV_offical_1771745119402.csv"
  );

  if (!fs.existsSync(csvPath)) {
    console.log("The Bedroom CSV not found, skipping");
    return;
  }
  ...
  console.log(`The Bedroom CSV: Parsed ${rows.length} rows`);
  ...
  try {
    ...
    const existingPL = await client.query(
      "SELECT id FROM price_lists WHERE name = 'The Bedroom'"
    );
    if (existingPL.rows.length > 0) {
      ...
      const newPL = await client.query(
        `INSERT INTO price_lists (name, description, is_default) VALUES ('The Bedroom', 'The Bedroom pricing', false) RETURNING id`
      );
      priceListId = newPL.rows[0].id;
    }
    console.log(`The Bedroom price list ID: ${priceListId}`);
    ...
    for (const [key, groupRows] of grouped) {
      ...
      if (!product) {
        ...
        const generatedSku = firstRow.sku || `BR-${Date.now()}-${newProductCount}`;
      ...
      for (const row of groupRows) {
        ...
        await client.query(
          ...
          [priceListId, product.id, row.filling || null, row.weight || null, row.price.toFixed(2)]
        );
        insertedCount++;
      }
    }
    ...
    console.log(
      `The Bedroom import complete: ${insertedCount} prices inserted, ${skippedCount} skipped, ${newProductCount} new products created`
    );
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("The Bedroom import error:", error);
    throw error;
  } finally {
    client.release();
  }
...
if (process.argv[1]?.includes("import-bedroom-pricelist")) {
  importBedroomPriceList()
    .then(() => process.exit(0))
