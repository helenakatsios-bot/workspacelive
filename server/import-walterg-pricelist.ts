...
export async function importWalterGPriceList() {
  const csvPath = path.join(
    process.cwd(),
    "attached_assets/prices_for_replit_watler_g_CSV_OFFICAL_1771749037859.csv"
  );

  if (!fs.existsSync(csvPath)) {
    console.log("Walter G CSV not found, skipping");
    return;
  }
  ...
  console.log(`Walter G CSV: Parsed ${rows.length} rows`);
  ...
  try {
    ...
    const existingPL = await client.query(
      "SELECT id FROM price_lists WHERE name = 'Walter G'"
    );
    if (existingPL.rows.length > 0) {
      ...
      if (parseInt(existingPrices.rows[0].cnt) > 0) {
        console.log("Walter G prices already imported, skipping");
        await client.query("ROLLBACK");
        return;
      }
    } else {
      const newPL = await client.query(
        `INSERT INTO price_lists (name, description, is_default) VALUES ('Walter G', 'Walter G pricing', false) RETURNING id`
      );
      priceListId = newPL.rows[0].id;
    }
    console.log(`Walter G price list ID: ${priceListId}`);
    ...
    for (const [key, groupRows] of grouped) {
      ...
      if (!product) {
        ...
        const generatedSku = firstRow.sku || `WALTER-${Date.now()}-${newProductCount}`;
    ...
    console.log(
      `Walter G import complete: ${insertedCount} prices inserted, ${skippedCount} skipped, ${newProductCount} new products created`
    );
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("Walter G import error:", error);
    throw error;
  } finally {
    client.release();
  }
...
if (process.argv[1]?.includes("import-walterg-pricelist")) {
  importWalterGPriceList()
    .then(() => process.exit(0))
