...
export async function importPoulosPriceList() {
  const csvPath = path.join(
    process.cwd(),
    "attached_assets/prices_for_replit_poulos_CSV_OFFICAL_1771744198218.csv"
  );

  if (!fs.existsSync(csvPath)) {
    console.log("Poulos CSV not found, skipping");
    return;
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const productName = cleanProductName((fields[0] || "").trim());
    const sku = (fields[1] || "").trim();
    const category = normalizeCategory((fields[2] || "").trim());
  ...
  console.log(`Poulos CSV: Parsed ${rows.length} rows`);
  ...
  try {
    ...
    if (existingPL.rows.length > 0) {
      ...
      const newPL = await client.query(
        `INSERT INTO price_lists (name, description, is_default) VALUES ('Poulos', 'Poulos pricing', false) RETURNING id`
      );
      priceListId = newPL.rows[0].id;
    }
    console.log(`Poulos price list ID: ${priceListId}`);
    ...
    for (const [key, groupRows] of grouped) {
      ...
      if (!product) {
        ...
        const result = await client.query(
          ...
          [firstRow.sku, name, category, basePrice]
        );
    ...
    console.log(
      `Poulos import complete: ${insertedCount} prices inserted, ${skippedCount} skipped, ${newProductCount} new products created`
    );
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("Poulos import error:", error);
    throw error;
  } finally {
    client.release();
  }
...
if (process.argv[1]?.includes("import-poulos-pricelist")) {
  importPoulosPriceList()
    .then(() => process.exit(0))
