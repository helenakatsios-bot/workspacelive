...
export async function importFrontlinePriceList() {
  ...
  if (!fs.existsSync(csvPath)) {
    console.log("Frontline CSV not found, skipping");
    return;
  }
  ...
  console.log(`Frontline CSV: Parsed ${rows.length} rows`);
  ...
  try {
    ...
    const existingPL = await client.query(
      "SELECT id FROM price_lists WHERE name = 'Frontline'"
    );
    if (existingPL.rows.length > 0) {
      ...
      const newPL = await client.query(
        `INSERT INTO price_lists (name, description, is_default) VALUES ('Frontline', 'Frontline pricing', false) RETURNING id`
      );
      priceListId = newPL.rows[0].id;
    }
    console.log(`Frontline price list ID: ${priceListId}`);
    ...
    for (const [key, groupRows] of grouped) {
      ...
      if (!product) {
        ...
        const generatedSku = firstRow.sku || `FL-${Date.now()}-${newProductCount}`;
    ...
    console.log(
      `Frontline import complete: ${insertedCount} prices inserted, ${skippedCount} skipped, ${newProductCount} new products created`
    );
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("Frontline import error:", error);
    throw error;
  } finally {
    client.release();
  }
...
if (process.argv[1]?.includes("import-frontline-pricelist")) {
  importFrontlinePriceList()
    .then(() => process.exit(0))
