...
export async function importSageClairePriceList() {
  const csvPath = path.join(
    process.cwd(),
    "attached_assets/price_for_replit_sage_&_claire_CSV_OFFICAL_1771744787359.csv"
  );

  if (!fs.existsSync(csvPath)) {
    console.log("Sage & Claire CSV not found, skipping");
    return;
  }
  ...
  console.log(`Sage & Claire CSV: Parsed ${rows.length} rows`);
  ...
  try {
    ...
    const existingPL = await client.query(
      "SELECT id FROM price_lists WHERE name = 'Sage & Claire'"
    );
    if (existingPL.rows.length > 0) {
      ...
      const newPL = await client.query(
        `INSERT INTO price_lists (name, description, is_default) VALUES ('Sage & Claire', 'Sage & Claire pricing', false) RETURNING id`
      );
      priceListId = newPL.rows[0].id;
    }
    console.log(`Sage & Claire price list ID: ${priceListId}`);
    ...
    console.log(
      `Sage & Claire import complete: ${insertedCount} prices inserted, ${skippedCount} skipped, ${newProductCount} new products created`
    );
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("Sage & Claire import error:", error);
    throw error;
  } finally {
    client.release();
  }
...
if (process.argv[1]?.includes("import-sageclaire-pricelist")) {
  importSageClairePriceList()
    .then(() => process.exit(0))
