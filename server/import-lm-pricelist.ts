...
export async function importLMPriceList() {
  const csvPath = path.join(
    process.cwd(),
    "attached_assets/prices_for_replit_L&M_CSV_OFFICAL_1771748512450.csv"
  );

  if (!fs.existsSync(csvPath)) {
    console.log("L&M CSV not found, skipping");
    return;
  }
  ...
  console.log(`L&M CSV: Parsed ${rows.length} rows`);
  ...
  try {
    ...
    const existingPL = await client.query(
      "SELECT id FROM price_lists WHERE name = 'L&M'"
    );
    if (existingPL.rows.length > 0) {
      ...
      if (parseInt(existingPrices.rows[0].cnt) > 0) {
        console.log("L&M prices already imported, skipping");
        await client.query("ROLLBACK");
        return;
      }
    } else {
      const newPL = await client.query(
        `INSERT INTO price_lists (name, description, is_default) VALUES ('L&M', 'L&M pricing', false) RETURNING id`
      );
      priceListId = newPL.rows[0].id;
    }
    console.log(`L&M price list ID: ${priceListId}`);
    ...
    for (const [key, groupRows] of grouped) {
      ...
      if (!product) {
        ...
        const generatedSku = firstRow.sku || `LM-${Date.now()}-${newProductCount}`;
    ...
    console.log(
      `L&M import complete: ${insertedCount} prices inserted, ${skippedCount} skipped, ${newProductCount} new products created`
    );
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("L&M import error:", error);
    throw error;
  } finally {
    client.release();
  }
...
if (process.argv[1]?.includes("import-lm-pricelist")) {
  importLMPriceList()
    .then(() => process.exit(0))
