...
export async function importHotelLuxuryPriceList() {
  const csvPath = path.join(
    process.cwd(),
    "attached_assets/prices_for_replit_Hotel_Luxury_collection_CSV_OFFICAL_1771748316614.csv"
  );

  if (!fs.existsSync(csvPath)) {
    console.log("Hotel Luxury Collection CSV not found, skipping");
    return;
  }
  ...
  console.log(`Hotel Luxury Collection CSV: Parsed ${rows.length} rows`);
  ...
  try {
    ...
    const existingPL = await client.query(
      "SELECT id FROM price_lists WHERE name = 'Hotel Luxury Collection'"
    );
    ...
    console.log(`Hotel Luxury Collection price list ID: ${priceListId}`);
    ...
    for (const [key, groupRows] of grouped) {
      ...
      if (!product) {
        ...
        const generatedSku = firstRow.sku || `HLC-${Date.now()}-${newProductCount}`;
    ...
    console.log(
      `Hotel Luxury Collection import complete: ${insertedCount} prices inserted, ${skippedCount} skipped, ${newProductCount} new products created`
    );
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("Hotel Luxury Collection import error:", error);
    throw error;
  } finally {
    client.release();
  }
...
if (process.argv[1]?.includes("import-hotelluxury-pricelist")) {
  importHotelLuxuryPriceList()
    .then(() => process.exit(0))
